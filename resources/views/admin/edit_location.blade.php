@extends('layouts/admin')

@section('content')
<h1>Add Location</h1>
<form method="POST" action="{{ url('/admin/updatelocation') }}/{{ $location->id }}" >
    @csrf
    <div class="mb-3">
      <label for="exampleInputEmail1" class="form-label">Parent</label>
      <select class="form-select" aria-label="Default select" name="parent_location">
          <option value="0">Top</option>
          @foreach ($locations as $location)
          <option value="{{ $location->id }}">{{ $location->location }}</option>
          @endforeach
        </select>
      <span class="text-danger">@error('category')
        {{ $message }}
      @enderror</span>
    </div>
    <div class="mb-3">
      <label for="exampleInputEmail1" class="form-label">Parent</label>
      <input type="text" class="form-control"  name="location"  value="{{ $location->location }}">
      
    </div>
    <div class="mb-3">
        <label for="exampleInputEmail1" class="form-label">Category</label>
        <input type="text" class="form-control" id="location" name="location"  value="{{ $location->location }}">
        
      </div>
      <div class="mb-3">
        <label for="exampleInputEmail1" class="form-label">Location Slug</label>
        <input type="text" class="form-control" id="location_slug" name="location_slug" placeholder="Location Slug" value="{{ $location->location_slug }}" >        
      </div>  
    
    <button type="submit" class="btn btn-primary">Submit</button>
  </form>

@endsection